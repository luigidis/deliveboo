<template>
    <header class="main_heder py-2 px-3 fixed w-full top-0 bg_text_color stroke_bottom z-50">
        <div class="container flex">
            <div class="logo_wrapper">
                <h1 class="logo text-3xl font-bold relative">
                    <router-link to="/" class="absolute w-full h-full" title="Vai alla home"></router-link>
                    Delive<span class="c_seco_color">Boo</span>
                </h1>
            </div>
            <nav class="navigation ml-auto">
                <ul class="flex items-center h-full gap-6">
                    <li>
                        <a href="/admin" class="font-normal text-xl" title="Accedi al tuo ristorante">
                            Il tuo ristorante
                        </a>
                    </li>
                    <li class="flex gap-1 justify-center items-center relative">
                        <span class="rounded-full bg-sky-300 totalItems">
                            {{ totalItems }}
                        </span>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512"><!--! Font Awesome Pro 6.2.1 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M0 24C0 10.7 10.7 0 24 0H69.5c22 0 41.5 12.8 50.6 32h411c26.3 0 45.5 25 38.6 50.4l-41 152.3c-8.5 31.4-37 53.3-69.5 53.3H170.7l5.4 28.5c2.2 11.3 12.1 19.5 23.6 19.5H488c13.3 0 24 10.7 24 24s-10.7 24-24 24H199.7c-34.6 0-64.3-24.6-70.7-58.5L77.4 54.5c-.7-3.8-4-6.5-7.9-6.5H24C10.7 48 0 37.3 0 24zM128 464a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zm336-48a48 48 0 1 1 0 96 48 48 0 1 1 0-96z"/></svg>
                        <router-link :to="{name: 'cart'}"
                        class="router" title="Vai al carrello"></router-link>
                    </li>
                </ul>
            </nav>
        </div>
    </header>
</template>
<script>
import state from "../store";


export default {
    name: 'TheHeader',
    computed: {
        totalItems() {
            return state.totalItems;
        }
    },
}
</script>
<style lang="scss" scoped>

    li svg {
        width: 25px;
        height: 25px;
    }

    .totalItems {
        display: flex;
        width: 2rem;
        height: 2rem;
        justify-content: center;
        align-items: center;
        font-weight: bold;
    }

    .router {
        position: absolute;
        top: 0;
        bottom: 0;
        right: 0;
        left: 0;
        cursor: pointer;
    }

</style>